...
2. 将 `chosen_password text := $password$YOUR_CHOSEN_DELETE_PASSWORD_HERE$password$;` 这一行中的占位文字替换为你选定的密码，再运行整份 SQL。只在 Supabase SQL Editor 中修改，别改本地文件，也不要把密码提交到 GitHub。脚本在未替换占位文字时会直接报错。
